...
void update_window(const SDL_t SDL, CHIP8_t *chip8);
void input(CHIP8_t *chip8);
void exec_instruction(CHIP8_t *chip8);
        ...
        while (SDL_PollEvent(&event)) {
                ...
                if (event.type == SDL_KEYDOWN || event.type == SDL_KEYUP) {
                        ...
                        case SDLK_k:
                                ...
                                break;
                        case SDLK_F5:
                                puts(save_state(chip8, "save_state.bin") ? "State saved." : "Save failed.");
                                break;
                        case SDLK_F9:
                                puts(load_state(chip8, "save_state.bin") ? "State loaded." : "Load failed.");
                                break;
